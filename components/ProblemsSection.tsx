...
export default function ProblemsSection({
  ...
  data: { heading: string; subheading?: string; items: { title: string; text: string }[]; tapeText?: string }
  showTape: boolean
}) {
    ...
    >
        ...
        <div style={{ marginTop: 'clamp(34px, 5vh, 56px)', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(270px, 1fr))', gap: 24 }}>
          {data.items.map((item, i) => (
            <Reveal key={i} className="af-problem-card">
              ...
              <p style={{ margin: '14px 0 0', fontSize: 16, lineHeight: 1.6, color: 'rgba(255,255,255,.78)' }}>{item.text}</p>
            </Reveal>
          ))}
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        .af-tape::after {
          ...
          right: 0;
          height: 7px;
          background: repeating-linear-gradient(90deg, #141414 0 16px, transparent 16px 40px);
        }
        .af-tape::before {
          top: 5px;
        }
