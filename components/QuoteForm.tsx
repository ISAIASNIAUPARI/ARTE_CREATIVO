 ...
 * texto con los datos y abre wa.me con ese número — igual en las 6 páginas del
 * sitio original. `inputStyle="light"` es la variante de fondo blanco que usan
 * todas las páginas salvo la portada (`dark`). `cardWrapper` reproduce la
 * tarjeta con borde que solo envuelve el formulario en la portada.
 */
export default function QuoteForm({
  ...
  const content = sent ? (
    ...
    <form onSubmit={onSubmit} style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 18 }}>
      ...
      {showErrorSlot && (
        <span style={{ gridColumn: '1 / -1', fontSize: 13, color: '#ff9f9f', minHeight: 18 }} />
      )}
