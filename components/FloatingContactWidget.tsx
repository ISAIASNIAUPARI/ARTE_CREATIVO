...
export default function FloatingContactWidget({ settings }: { settings: SiteSettings }) {
    ...
    fetch(settings.chatWebhookUrl, {
      ...
      .catch(() => {
        setLog((l) => [
          ...
          {
            from: 'bot',
            text: 'Ahora mismo no puedo responder por aquí. Escríbeme por WhatsApp y te contesto directo.',
          },
        ])
        setThinking(false)
        setTimeout(scrollLog, 60)
  ...
  const quick = settings.chatQuickReplies?.length ? settings.chatQuickReplies : DEFAULT_QUICK
  const botAvatarUrl = settings.chatBotAvatar ? urlFor(settings.chatBotAvatar).size(88, 88).url() : null
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: '#1c1c1c' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 3, flex: 1, minWidth: 0 }}>
              <span style={{ fontFamily: "'Exo', sans-serif", fontWeight: 700, fontSize: 17, color: '#ffffff' }}>
                Asistente Artly
              </span>
      ...
      {launcherOpen && (
        ...
        >
          ...
          {settings.chatEnabled && (
            <button type="button" onClick={() => { setLauncherOpen(false); setChatOpen(true) }} className="af-launcher-card af-launcher-bot">
              {botAvatarUrl && (
                <Image
                  src={botAvatarUrl}
                  alt="Asistente Artly"
